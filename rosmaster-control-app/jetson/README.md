...
本目录把原桌面快捷指令 `app` 的底盘控制和寻迹开关提供给新的 Android App。它直接调用主机 `Rosmaster_Lib`，不依赖 ROS、Docker 或容器，也不修改 `/home/jetson/Rosmaster-App/rosmaster`。默认启动脚本只监听 9093，摄像头和识别视频由 9095 人脸服务负责。

默认地址：

- WebSocket：`ws://<Jetson-IP>:9093`
- MJPEG：默认禁用；仅兼容旧方案时使用 `http://<Jetson-IP>:9094/video.mjpg`
...
`start_bridge.sh` 和 `scripts/start.sh` 都默认设置 `NO_VIDEO=1`：不打开 `/dev/video*`，也不监听 9094，只启动 9093。这样 9095 可以独占摄像头。
...
NO_VIDEO=1 bash scripts/start.sh
VENDOR_ROOT=/home/jetson/Rosmaster-App/rosmaster bash scripts/start.sh
```

如需临时恢复旧的 9094 原始视频模式，可显式执行：

```bash
NO_VIDEO=0 bash start_bridge.sh
```
...
python bridge_server.py --mock --no-video --host 127.0.0.1
```
