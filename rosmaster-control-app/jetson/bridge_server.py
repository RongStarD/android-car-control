...
def build_parser():
    ...
    parser.add_argument(
        "--no-video",
        action="store_true",
        help="do not open a camera or listen on the MJPEG video port",
    )
    return parser
...
async def run(args):
    ...
    camera = DisabledCamera() if args.no_video or args.no_camera or args.mock else CameraCapture(
    ...
    video_server = None if args.no_video else MjpegServer(args.host, args.video_port, camera)
    ...
    try:
        ...
        if video_server is None:
            print("控制桥已启动：WebSocket {}，视频已禁用".format(args.ws_port), flush=True)
        else:
...
def main():
    ...
    if not (1 <= args.ws_port <= 65535) or (
        not args.no_video and not (1 <= args.video_port <= 65535)
    ):
        print("端口必须在 1 到 65535 之间", file=sys.stderr); return 2
    if not args.no_video and args.ws_port == args.video_port:
