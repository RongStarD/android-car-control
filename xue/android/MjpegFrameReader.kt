...
/** Reads bounded JPEG frames from a multipart MJPEG response. */
internal class MjpegFrameReader(
    ...
    private fun readLengthDelimitedFrame(length: Int): ByteArray {
        if (length !in 2..maxFrameBytes) throw IOException("Invalid MJPEG frame length: $length")
        val frame = source.readByteArray(length.toLong())
        if (!frame.hasJpegMarkers()) throw IOException("MJPEG part is not a JPEG image")
        return frame
    }
    ...
    private fun readMarkerDelimitedFrame(): ByteArray? {
        ...
        while (!source.exhausted()) {
            ...
            if (frame.size > maxFrameBytes) throw IOException("MJPEG frame exceeds $maxFrameBytes bytes")
            if (previous == JPEG_MARKER_PREFIX && current == JPEG_END) return frame.toByteArray()
            previous = current
        }
        return null
    }
